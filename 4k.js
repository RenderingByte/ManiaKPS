...
window.onload = function () {
    ...
    // load keybinds from storage and update UI
    for (var i = 0; i < keybinds.length; i++) {
        if (localStorage.getItem("keybind" + (i).toString()) != null) {
            let KeyElem = document.getElementById("Key" + (i+1).toString());
            keybinds[i] = localStorage.getItem("keybind" + (i).toString());
            KeyElem.value = keybinds[i];
        }
    }
...
function new_keybind(event, key_element)
{
    ...
    else
    {
        ...
        localStorage.setItem("keybind" + (key_element-1).toString(), keybinds[key_element - 1]);
    }
}
...
function run_timer()
{
    ...
    function run_timer_()
    {
        ...
        bpm = ((kps/(keybinds.length)*2) * 30).toFixed(1);
        ...
        if (timer % 100 == 0)
        {
            this.updateChart(dataLength, timer/100, presses/(timer/1000));
        }
...
function run_clicks()
{
    ...
    function run_click_timer()
    {
        ...
        if (timer % 100 == 0) { this.updateChart(dataLength, timer/100, presses/(timer/1000)); }
